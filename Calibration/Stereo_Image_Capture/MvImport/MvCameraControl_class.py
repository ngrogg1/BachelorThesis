...
# 用于回调函数传入相机实例 callback camera object
...
class MvCamera():
    ...
    def __init__(self):
        self._handle = c_void_p()  # 记录当前连接设备的句柄 record the handle
        self.handle = pointer(self._handle)  # 创建句柄指针 create pointer for handle
